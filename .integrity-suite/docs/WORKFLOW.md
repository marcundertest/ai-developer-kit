...
- El agente implementa el código para que cumpla con los tests, y los tests para que cumplan con los requerimientos
  del usuario.
- El agente NO realiza commits ni push de forma autónoma bajo ninguna circunstancia.
- **Sugerencia de commit**: Una vez que la tarea está terminada y todos los tests (incluyendo la Integrity Suite) pasan al 100%, el agente debe marcar el requerimiento como **Completado** en `requirements.md` y sugerir un mensaje de commit al usuario en el chat.
- El usuario es el único responsable de efectuar el `git commit` y `git push` manualmente desde su terminal después de revisar los cambios.
- La Integrity Suite sirve como guardián local: si el agente intenta sugerir un mensaje de commit sin haber marcado el requerimiento como **Completado** o sin pasar los tests, el usuario debe rechazarlo.
- El agente NO tiene herramientas para realizar operaciones de escritura en Git.
